...
export default function ChatUI({ bot }) {
  ...
  const handleBotSelect = (bot) => {
    setSelectedBot(bot);
    if (!localStorage.getItem("age_verified")) setShowAgeModal(true);
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-[#2C1F3D] text-white">
      <div className="bg-[#1F1B29] p-3 shadow-lg flex justify-between items-center">
        <h1 className="text-xl font-bold text-white text-center sm:text-lg md:text-xl">
          VOXELLA AI
        </h1>
        {!isAuthenticated && (
      ...
      <div className="flex-1 overflow-y-auto p-2 sm:p-1">
        {messages.map((msg, index) => (
          <motion.div
            ...
            className={`flex items-end mb-2 sm:mb-1 ${
